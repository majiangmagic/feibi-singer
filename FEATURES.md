...
- 验证步骤：
  ...
  4. 由用户试听确认后完成验收。。
- 验证证据：20 项 pytest 通过；候选搜索不再在最低 85% 门槛处提前停止，而是优先寻找 100% 覆盖率。新版 unhappy 5 段所选 seed 为 44、46、49、48、49，覆盖率均为 100%；第 4 段 56.7-70.4 秒未检测到持续 0.25 秒静音。最终 WAV 为 98.246542 秒、48 kHz、24-bit 立体声、峰值 -1.0 dBFS，MP3 为 320 kbps。用户试听确认连续性改善，但“菲比”发音仍有偏离，暂不验收。。
- 备注：已确认 seed 42 质量差并废弃；采用 7.5 秒上下文、0.25 秒拼接把手和 seed 44、43、45-50 候选池。所有片段固定升 4 半音；逐秒覆盖率只用于防漏唱，不能评价目标词发音。下一步需定位发音偏差发生在 ACE 还是 RVC，并增加发音约束或可试听候选对照。。
