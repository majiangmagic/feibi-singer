...
- 验证步骤：
  ...
  4. 由用户试听确认后完成验收。。
- 验证证据：19 项 pytest 通过；unhappy v4 的 5 段所选 ACE 人声逐秒 RMS 覆盖率依次为 100%、92.3%、100%、85.7%、100%，统一 RVC f0_change +4；最终 WAV 为 98.246542 秒、48 kHz、24-bit 立体声、峰值 -1.0 dBFS，MP3 为 320 kbps，-45 dB 持续 1 秒检测无长静音，等待用户试听。。
- 备注：已确认 seed 42 质量差并废弃；采用 7.5 秒上下文、0.25 秒拼接把手和 seed 44、43、45-50 候选池。Wav2Vec2 性别模型误判原曲男声，自动性别升调方案已移除，所有片段固定升 4 半音。。
