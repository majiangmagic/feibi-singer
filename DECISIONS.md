...
## RVC 默认升调采用 +6 并保留覆盖能力

- 索引：RVC, 默认升调, +6, 分段覆盖, UI, CLI
- 日期：2026-08-14
- 状态：adopted
- 决策：将时间轴一次性生成、分段工作台缺省值、UI 滑块初始值和独立 RVC 推理器的默认升调统一为 +6 半音；显式的每段或每次运行覆盖仍然优先。
- 原因：用户对 +6 版本进行试听验证后，将其作为之后生成器的默认起点，同时保留逐段试音和覆盖能力。
- 放弃方案：不把已有 report 的显式升调强行改写为 +6，也不取消用户对单段 RVC 升调的覆盖。
- 影响：未来新生成任务默认使用 +6；旧 report 中已记录的 rvc_f0_change 继续保持原值，新的工作台状态在缺少该字段时回退到 +6。
