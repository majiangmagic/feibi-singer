...
- `feibi_singer/timeline_pipeline.py`：默认真实执行器，检测原唱窗口、截取窗口生成、二次分离、RVC 歌声转换、延迟对齐并混回原始伴奏；
...
- `scripts/feibi_ace_step_v15.py`：ACE-Step 1.5 非交互 cover engine，支持 INT8、CPU offload、源音频、新歌词和确定性 seed；
...
- 默认真实方案必须保留原始伴奏，并按原分离人声检测到的起止区间对齐生成歌声；旧整曲直通方案只能显式启用；
